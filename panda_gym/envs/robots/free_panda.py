...
class FreePanda(PyBulletRobot):
    ...
    def __init__(
        ...
        n_action = 6 if self.block_gripper else 7
    ...
    def set_action(self, action):
        ...
        ee_orientation_ctrl = action[3:6] * 0.1  # limit maximum change in orientation
        ee_orientation = p.getEulerFromQuaternion(self.get_ee_orientation())
        target_ee_orientation = p.getQuaternionFromEuler(ee_orientation + ee_orientation_ctrl)

        # compute the new joint angles
        target_angles = self._inverse_kinematics(
            position=target_ee_position, orientation=target_ee_orientation
        )

        if not self.block_gripper:
            fingers_ctrl = action[6] * 0.2  # limit maximum change in position
    ...
    def get_ee_angular_velocity(self):
        return self.get_link_angular_velocity(self.ee_link)
